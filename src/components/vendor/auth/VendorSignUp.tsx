// src/components/vendor/auth/VendorSignUp.tsx
...
const VendorSignUp: React.FC = () => {
	...
	useEffect(() => {
		if (!initializing) {
			setAuthChecked(true);
			if (user) {
				// If email verified, go to vendor dashboard
	...
	const handleLocationSelect = (locationData: {
		...
		// Store coordinates - create a simple object with primitive values
		const coordsData = {
			lat: locationData.lat,
			lon: locationData.lon,
		};

		// Store as string versions to avoid the enumeration issue
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (imageFile) {
				...
				// Create a path for the image: vendors/[timestamp]-[filename]
				const timestamp = Date.now();
				const path = `vendors/signup/${timestamp}-${imageFile.name}`;
			...
			// Create coordinates object with simple primitive values
			...
			// Create vendor account with all required information
			...
			// If vendor creation was successful, update the profile with the image URL
			if (result.success && result.user && profileImageUrl) {
				// Update the vendor profile to include the profile image
				await updateVendorProfile({
					profileImage: profileImageUrl,
				});
			}

			// SUCCESS - Add a delay before redirecting to ensure Firestore data is fully created
			setErrorMessage(
				"Account created successfully! Redirecting to dashboard..."
			);

			// Wait 3 seconds before redirecting to ensure Firestore data is ready
			setTimeout(() => {
				router.replace("/vendor/dashboard");
			}, 3000);
		} catch (error) {
	...
	return (
		<div className="bg-white p-6 rounded-lg shadow-md">
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="mb-4">
					...
					<div className="mb-4">
						...
						<input
							id="shopName"
							type="text"
							className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-puce focus:border-transparent ${
								formTouched.shopName && !shopName
									? "border-red-500"
									: formTouched.shopName && shopName
									? "border-green-500"
									: "border-gray-300"
							...
							onBlur={() => setFormTouched({ ...formTouched, shopName: true })}
							required
						/>
						{formTouched.shopName && !shopName && (
							<p className="mt-1 text-sm text-red-500">
								Shop/Restaurant name is required
							</p>
						)}
					</div>
					...
					<div className="mb-4">
						...
						<GoogleMapsAutocomplete
							...
							countryCode="us"
						/>
						{formTouched.location && !addressSelected && !coordinates ? (
							<p className="mt-1 text-sm text-red-500">
					...
					<div className="mb-4">
						...
						<p className="mt-2 text-xs text-gray-500">
							Set your business hours for each day of the week. If you&apos;re
							closed on a particular day, select &quot;Closed&quot; and you can
							add a reason which will be shown to customers.
						</p>
					</div>
				</div>
				...
				<button
					type="submit"
					className="w-full bg-puce cursor-pointer hover:bg-rose text-white py-3 px-4 rounded-md text-lg font-medium shadow-md transition-colors duration-200"
					disabled={
						isLoading ||
						isUploading ||
